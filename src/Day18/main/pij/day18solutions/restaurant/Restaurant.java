...
public class Restaurant {
    ...
    /**
     * The executor service that takes the role of the kitchen with a fixed
     * number of cooks for our restaurant in meal preparation. Non-null.
     */
    ...
    public Restaurant(int nCooks) {
        // A fixed thread pool provides the best match for the application
        // requirement that one cook can make at most one meal at a time.
        this.kitchen = Executors.newFixedThreadPool(nCooks);
    }
    ...
    public synchronized Future<Meal> order(String dishName) {
        // we could also use a lambda expression instead of creating
        // a class MealOrder
        Callable<Meal> order = new MealOrder(dishName);
        Future<Meal> result = this.kitchen.submit(order);
        return result;
    }
