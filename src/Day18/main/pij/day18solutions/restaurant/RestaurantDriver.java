...
public class RestaurantDriver {
    ...
    public static void main(String[] args) {
        ...
        while (! futures.isEmpty()) {
            ...
            // we need an Iterator object because enhanced for-loops and
            // streams cannot delete from a list
            Iterator<Future<Meal>> iter = futures.iterator();
            while (iter.hasNext()) {
                Future<Meal> nextFuture = iter.next();
                if (nextFuture.isDone()) { // is the meal ready?
                    iter.remove(); // we won't need to revisit nextFuture
                    try {
        ...
        // don't forget to send the cooks home (otherwise the application will
        // not terminate)
        restaurant.shutdown();
    }
}
