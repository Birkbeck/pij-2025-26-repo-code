package pij.day14solutions.tree;
...
public class TreeNode<E> {
    ...
    /**
     * Removes the subtree at position p if this is possible. If the subtree's
     * root was an inner node, the tree structure of this tree may be modified
     * in an arbitrary way.
     *
     * @param p the position at which to remove
     * @return true (the spec was unclear on the intended return value)
     * @throws NullPointerException if p is null
     * @throws IllegalArgumentException if this tree does not have p as a
     *  position
     */
    public E remove(Position p) {
        ...
        Direction dir = p.get(0);
        E result = null;
        if (pSize == 1) { // remove a child below the current node
            switch (dir) {
                case LEFT:
                    if (this.left == null) {
                        throw new IllegalArgumentException("Nothing to remove here!");
                    }
                    result = this.left.data;
                    // now we have up to two subtrees below this.left;
                    // they both need new homes
                    if (this.left.left == null) {
                        // there is no left child, so the right child takes the
                        // place of the deleted node
                        this.left = this.left.right;
                    } else {
                        // the left child takes the place of the deleted node
                        if (this.left.right != null) {
                            // the right child (if any) first needs to go
                            // below the left child
                            this.left.left.appendLeftmostSubtree(this.left.right);
                        }
                ...
                case RIGHT:
                    ...
                    result = this.right.data;
                    // now we have up to two subtrees below this.right;
                    // they both need new homes
                    if (this.right.left == null) {
                        ...
                        if (this.right.right != null) {
                            // the right child (if any) first needs to go
                            // below the left child
                            this.right.left.appendLeftmostSubtree(this.left.right);
                        }
        ...
        switch (dir) {
            case LEFT:
                ...
                return this.left.remove(smallerPosition);
            case RIGHT:
                if (this.right == null) {
                    throw new IllegalArgumentException("Remaining path empty, but remaining position is "
                            + smallerPosition);
                }
                return this.right.remove(smallerPosition);
            default:
                // If someone has added another entry to enum Direction
                // in the meantime, this code will have to be revisited.
                throw new RuntimeException("Unknown direction " + dir + "!");
    ...
    /**
     * Inserts subtree below the leftmost position of this tree.
     *
     * @param subtree the subtree to insert
     */
    private void appendLeftmostSubtree(TreeNode<E> subtree) {
